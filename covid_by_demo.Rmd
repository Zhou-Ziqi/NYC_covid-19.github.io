...
a = plot_ly(labels = paste0("Age Group: ", data_to_plot$group),
...
b = plot_ly(labels = paste0("Age Group: ", data_to_plot$group),
            values = data_to_plot$count,
            type = "pie",
            marker = list(colors = brewer.pal(7,"Blues")),
            sort = FALSE,
...
data_to_plot2 = byage %>% 
  filter(day == "2020-06-30" & 
           boro == "BK" & 
           group != "Boroughwide" &
           outcome == "HOSPITALIZED_RATE") 

t = plot_ly(labels = paste0("Age Group: ", data_to_plot2$group),
                    values = data_to_plot2$count,
                    type = "pie",
                    marker = list(colors = brewer.pal(7,"Blues")),
                    sort = FALSE,
                    opacity=0.8) %>% 
...
e = plot_ly(labels = paste0("Age Group: ", data_to_plot3$group),
                    values = data_to_plot3$count,
                    type = "pie",
                    marker = list(colors = brewer.pal(7,"Blues")),
                    sort = FALSE,
                    opacity=0.8) %>% 
...
pie1data = pie1data[order(pie1data$count, decreasing = TRUE),]
...
pie2data = byage %>% filter(boro == "BK"& day == max(byage$day) &group != "Boroughwide" & outcome =="CASE_COUNT")

pie3data = byage %>% filter(boro == "MN"& day == max(byage$day) &group != "Boroughwide" & outcome =="CASE_COUNT")


pie4data = byage %>% filter(boro == "SI"& day == max(byage$day) &group != "Boroughwide" & outcome =="CASE_COUNT")


pie5data = byage %>% filter(boro == "QN"& day == max(byage$day) &group != "Boroughwide" & outcome =="CASE_COUNT")
