...
newdata = read_csv(paste0("./data/modzcta_until",month(yesterday),day(yesterday),".csv")) %>% 
...
```{r}
byage = read_csv(paste0("./distribution_of_covid-19/data/demoage_until",month(yesterday),day(yesterday),".csv")) # 
...
#### by race
```{r}
byrace = read_csv(paste0("./distribution_of_covid-19/data/demorace_until",month(yesterday),day(yesterday),".csv"))
...
bysex_new = read_csv("./data/data_covid_demo/boros_by_sex/boroughs-by-sex0819.csv") %>% 
  pivot_longer(BK_CASE_COUNT:SI_DEATH_RATE,
               names_to = "location_n_event",
               values_to = "count")%>% 
  mutate(day = today,
...
write_csv(byrace,paste0("./distribution_of_covid-19/data/demosex_until",month(today),day(today),".csv")) 
